...
internal static class DirectCallBack
{
    ...
    private static void StartClient(string host, int id)
    {
        ...
        state.WorkSocket.BeginConnect(state.ipEndPoint, OnConnect, state);
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        try
        {
            ...
            if (bytesRead > 0)
            {
                ...
                if (!HttpUtils.responseHeaderFullyObtained(state.StringBuilder.ToString()))
                {
                ...
                else
                {
                    ...
                    var responseBody = HttpUtils.getResponseBody(state.StringBuilder.ToString());
                    ...
                    var contentLengthHeaderValue = HttpUtils.getContentLength(state.StringBuilder.ToString());
